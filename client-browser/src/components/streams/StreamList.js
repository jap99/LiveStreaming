...
class StreamList extends React.Component {      // class based so we can call action creator in componentDidMount
  ...
  renderDeleteEditBtns(stream) {  // TODO ------ this should probably be on server side also so hackers can't delete other people's streams
    ...
    return this.props.streams.map(stream => {
      return (
        <div className="item" key={ stream.id }>
          { this.renderDeleteEditBtns(stream) }
...
const mapStateToProps = (state) => {                       // Object.values takes all the values of an object & puts them in an array
  return {  
